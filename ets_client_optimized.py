...
def get_active_network_interface():
    """
    获取当前活跃网络接口的MAC地址
    通过发送网络请求到外部服务器来确定使用的网卡
    """
    try:
        # 创建一个UDP socket连接到外部服务器
        # 这不会发送实际数据，但会确定路由使用的网络接口
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
            # 连接到114.114.114.114的DNS端口
        ...
        logger.error(f"获取活跃网络接口MAC地址失败: {e}")
        return ""
...
def get_mac_address():
    ...
    try:
        # 首先尝试获取活跃网络接口的MAC地址
        active_mac = get_active_network_interface()
        if active_mac:
            return active_mac
...
def main():
    """主函数"""
    print("欢迎使用作业答案生成器v1.1(Release - 优化MAC地址获取)")
    ...
    print("优化说明：现在使用网络请求确定活跃网卡的MAC地址")
...
if __name__ == "__main__":
    ...
    try:
        # 检查命令行参数
        if len(sys.argv) > 1 and sys.argv[1] == "--test-hwid":
            test_hwid()
            input("\n按任意键退出...")
        else:
            main()
